...
function LevelBadge({ level, kind }: { level: Level; kind: 'demand' | 'coverage' }) {
  ...
  return <Badge variant={variant} className="font-bold text-[10px] px-2 py-0.5">{level}</Badge>
}
            ...
            <div key={s.skill} className="flex flex-col gap-2 rounded-xl border border-slate-200/80 bg-slate-50/70 p-3.5 shadow-2xs hover:border-slate-300 transition-colors">
              ...
              <div className="flex flex-col gap-1.5 pt-1">
                <div className="flex items-center gap-2.5">
                  <span className="w-16 shrink-0 text-xs font-semibold text-slate-700">Demand</span>
                  ...
                  <span className="w-8 shrink-0 text-right text-xs font-bold tabular-nums text-slate-950">
                    {s.demandScore}
                  </span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="w-16 shrink-0 text-xs font-semibold text-slate-700">Coverage</span>
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-200">
                    <div
